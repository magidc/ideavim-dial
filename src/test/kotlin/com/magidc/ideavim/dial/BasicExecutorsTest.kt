...
class BasicExecutorsTest : BaseTest() {
  ...
  fun testNoMatchForPartialWords() {
    ...
    assertThat(execute("if (something" + CARET + "true)"))
      .isEqualTo("if (something" + CARET + "true)")
    assertThat(execute("if (truly" + CARET + "False)")).isEqualTo("if (truly" + CARET + "False)")
  }

  fun testMultipleOccurrencesChoosesClosestToCursor() {
    assertThat(execute("if (true && " + CARET + "true)"))
      .isEqualTo("if (true && " + CARET + "false)")
    assertThat(execute("if (" + CARET + "true && false)"))
      .isEqualTo("if (" + CARET + "false && false)")
  }
  ...
  fun testLogLevels() {
    ...
    assertThat(execute("logger.d" + CARET + "ebug('message')"))
      .isEqualTo("logger." + CARET + "info('message')")
    assertThat(execute("logger.i" + CARET + "nfo('message')"))
      .isEqualTo("logger." + CARET + "warning('message')")
  }
}
