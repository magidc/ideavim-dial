...
class DateExecutorsTest : BaseTest() {
  ...
  companion object {
    val testDateSet: List<ZonedDateTime> =
      listOf(
  ...
  private fun testDateTimes(datePattern: String, temporalUnit: TemporalUnit, locale: Locale) {
    ...
    for (date in testDateSet) {
      ...
      assertThat(execute(CARET + "value = $formattedDate"))
        .isEqualTo("value = $CARET$formattedDatePlusOne")
      assertThat(execute("value =$CARET $formattedDate"))
        .isEqualTo("value = $CARET$formattedDatePlusOne")
      assertThat(execute("value = $formattedDate$CARET")).isEqualTo("value = $formattedDate$CARET")
      assertThat(execute("value$CARET = $formattedDate"))
        .isEqualTo("value = $CARET$formattedDatePlusOne")

      assertThat(execute("value = ${formattedDate.take(3)}" + CARET + formattedDate.substring(3)))
        .isEqualTo("value = $CARET$formattedDatePlusOne")
      // Test decrementing the date
      val formattedDateMinusOne = formatter.format(date.minus(1, temporalUnit))
      assertThat(execute(CARET + "value = $formattedDate", true))
        .isEqualTo("value = $CARET$formattedDateMinusOne")
      assertThat(execute("value =$CARET $formattedDate", true))
        .isEqualTo("value = $CARET$formattedDateMinusOne")
      assertThat(execute("value = $formattedDate$CARET", true))
        .isEqualTo("value = $formattedDate$CARET")
      assertThat(execute("value$CARET = $formattedDate", true))
        .isEqualTo("value = $CARET$formattedDateMinusOne")

      assertThat(
          execute("value = ${formattedDate.take(3)}" + CARET + formattedDate.substring(3), true)
        )
        .isEqualTo("value = $CARET$formattedDateMinusOne")
    }
  }
