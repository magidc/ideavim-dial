...
class MarkdownExecutorsTest : BaseTest() {
  ...
  fun testTasksWithIndentation() {
    ...
    assertThat(execute("    - [" + CARET + "x] Double indented"))
  ...
  fun testTasksWithVariousContent() {
    assertThat(execute("- [$CARET ] Task with * special ^ characters"))
      .isEqualTo("$CARET- [x] Task with * special ^ characters")
    assertThat(execute("- [" + CARET + "x] Task with [link](url)"))
      .isEqualTo("$CARET- [ ] Task with [link](url)")
    assertThat(execute("- [$CARET ] Task with **bold** and *italic*"))
