...
class PythonExecutorsTest : BaseTest() {
    ...
    fun testComparisonOperators() {
        ...
        assertThat(execute("if obj " + CARET + "is None:")).isEqualTo("if obj " + CARET + "is not None:")

        // not in / in
        assertThat(execute("if item " + CARET + "not in collection:")).isEqualTo("if item " + CARET + "in collection:")
        assertThat(execute("if key " + CARET + "in dictionary:")).isEqualTo("if key " + CARET + "not in dictionary:")
    ...
    fun testLoopTypes() {
        ...
        // Test with different caret positions
        assertThat(execute("fo" + CARET + "r i in range(10):")).isEqualTo(CARET + "while i in range(10):")
    ...
    fun testLoopControl() {
        ...
        assertThat(execute("if not ready: " + CARET + "continue")).isEqualTo("if not ready: " + CARET + "break")
    }
    ...
    fun testMethods() {
        ...
        // Test with different caret positions
        assertThat(execute("de" + CARET + "f my_function():")).isEqualTo(CARET + "async def my_function():")
        assertThat(execute("async de" + CARET + "f handler():")).isEqualTo(CARET + "def handler():")
    }

    fun testAsyncKeywords() {
        assertThat(execute(CARET + "with open('file') as f:")).isEqualTo(CARET + "async with open('file') as f:")
        assertThat(execute(CARET + "async with aiofiles.open('file') as f:")).isEqualTo(CARET + "with aiofiles.open('file') as f:")

        // Test with different caret positions
        assertThat(execute("wi" + CARET + "th context_manager():")).isEqualTo(CARET + "async with context_manager():")
    }
    ...
    fun testCollectionOperations() {
        ...
        assertThat(execute("lst" + CARET + ".extend(items)")).isEqualTo("lst" + CARET + ".insert(items)")
        assertThat(execute("lst" + CARET + ".insert(0, item)")).isEqualTo("lst" + CARET + ".remove(0, item)")
        ...
        assertThat(execute("items" + CARET + ".append('new')")).isEqualTo("items" + CARET + ".extend('new')")
    ...
    fun testStringCaseMethods() {
        ...
        // Test in different contexts
        assertThat(execute("name" + CARET + ".upper().strip()")).isEqualTo("name" + CARET + ".lower().strip()")
        assertThat(execute("return word" + CARET + ".lower()")).isEqualTo("return word" + CARET + ".upper()")
    }
    ...
    fun testAssertions() {
        ...
        assertThat(execute(CARET + "assertIn(item, container)")).isEqualTo(CARET + "assertNotIn(item, container)")
    ...
    fun testStringQuotes() {
        assertThat(execute("text = " + CARET + "\"hello world\"")).isEqualTo("text = " + CARET + "'hello world'")
        assertThat(execute("text = " + CARET + "'hello world'")).isEqualTo("text = " + CARET + "\"hello world\"")

        // Test with complex strings
        assertThat(execute("msg = " + CARET + "\"This is a test\"")).isEqualTo("msg = " + CARET + "'This is a test'")
        assertThat(execute("path = " + CARET + "'/home/user/file'")).isEqualTo("path = " + CARET + "\"/home/user/file\"")

        // Test with different caret positions
        assertThat(execute("name = \"Joh" + CARET + "n Doe\"")).isEqualTo("name = \"Joh" + CARET + "n Doe\"")
        assertThat(execute("city = 'New Yor" + CARET + "k'")).isEqualTo("city = 'New Yor" + CARET + "k'")
    }
    ...
    fun testLogicalOperators() {
        ...
        assertThat(execute("while condition o" + CARET + "r flag:")).isEqualTo("while condition " + CARET + "and flag:")
    }

    fun testMultipleOccurrencesChoosesClosestToCursor() {
        assertThat(execute("if True and " + CARET + "True:")).isEqualTo("if True and " + CARET + "False:")
        assertThat(execute("list.append(x); " + CARET + "list.append(y)")).isEqualTo("list.append(x); list$CARET.extend(y)")
        assertThat(execute("def func(): pass\n" + CARET + "def other():")).isEqualTo("def func(): pass\n" + CARET + "async def other():")
    }

    fun testNoMatchForPartialWords() {
        // Should not match parts of larger words
        assertThat(execute("mylist" + CARET + "comprehension")).isEqualTo("mylist" + CARET + "comprehension")
    ...
    fun testWithSurroundingPunctuation() {
        ...
        assertThat(execute("result = [" + CARET + "list()]")).isEqualTo("result = [" + CARET + "tuple()]")
        assertThat(execute("types = {'" + CARET + "int': 42}")).isEqualTo("types = {'" + CARET + "float': 42}")
        assertThat(execute("obj" + CARET + ".append(item);")).isEqualTo("obj" + CARET + ".extend(item);")
    }

    fun testMethodChaining() {
        assertThat(execute("text.strip()" + CARET + ".upper().replace(' ', '_')")).isEqualTo("text.strip()" + CARET + ".lower().replace(' ', '_')")
        assertThat(execute("data.filter(lambda x: x > 0)" + CARET + ".append(item)")).isEqualTo("data.filter(lambda x: x > 0)" + CARET + ".extend(item)")
    }

    fun testComplexExpressions() {
        // Test within complex expressions
        assertThat(execute("if obj " + CARET + "is not None and len(obj) > 0:")).isEqualTo("if obj " + CARET + "is None and len(obj) > 0:")
        assertThat(execute("return [item " + CARET + "for item in items if item is not None]")).isEqualTo("return [item " + CARET + "while item in items if item is not None]")
        assertThat(execute("result = text$CARET.lower() if text else ''")).isEqualTo("result = text$CARET.upper() if text else ''")
    }

    fun testFunctionDefinitions() {
        assertThat(execute(CARET + "def process_data(data):")).isEqualTo(CARET + "async def process_data(data):")
        assertThat(execute(CARET + "async def fetch_data(url):")).isEqualTo(CARET + "def fetch_data(url):")

        // With decorators
        assertThat(execute("@decorator\n" + CARET + "def method(self):")).isEqualTo("@decorator\n" + CARET + "async def method(self):")
    }

    fun testAsyncContexts() {
        assertThat(execute(CARET + "with session.get(url) as response:")).isEqualTo(CARET + "async with session.get(url) as response:")
        assertThat(execute(CARET + "async with asyncio.Lock():")).isEqualTo(CARET + "with asyncio.Lock():")
    }
}
