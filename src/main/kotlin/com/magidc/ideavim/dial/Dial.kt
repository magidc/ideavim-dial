...
class Dial : VimExtension {
  companion object {
    ...
    @NonNls const val DIAL_INCLUDED_DEFINITIONS_VARIABLE_NAME = "dial_include"

    @NonNls const val DIAL_CUSTOM_DEFINITIONS_VARIABLE_NAME = "dial_custom"
  }
...
fun createWordSet(words: List<VimDataType>): VimList = buildPattern(words) { word -> "\\b$word\\b" }

fun createNormalizedCasePattern(words: List<VimDataType>): VimList =
  buildPattern(words) { word -> "(?i)$word" }

fun createPattern(words: List<VimDataType>): VimList = buildPattern(words) { word -> word }
