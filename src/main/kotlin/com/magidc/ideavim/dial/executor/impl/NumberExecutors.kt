...
object NumberExecutors : ExecutorProvider {
    ...
    private fun incrementVersionCode(matchResult: MatchResult, increment: Boolean = true): String? {
        val tokens = matchResult.value.split(VERSION_CODE_REGEX).filter { it.isNotEmpty() }.reversed().toMutableList()
        ...
        for (i in tokens.indices) {
            try {
                ...
                if (!increment && originalValue.signum() == 0) {
                    ...
                    tokens[i] = (incrementWithPrecision(token, increment) ?: return null)
    ...
    private fun incrementWithPrecision(value: String?, increment: Boolean = true): String? {
        ...
        return if (decimalIndex != -1) {
            ...
            val newValue = if (increment) currentValue.add(precisionStep) else currentValue.subtract(precisionStep)
