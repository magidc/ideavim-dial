...
open class Executor(
    ...
    val matchBefore: Boolean = true,
    val reusable: Boolean = true, // Whether the executor can be applied to the replacements generated by itself
    ...
    open fun findMatch(text: String, cursorOffset: Int, reverse: Boolean): Match? {
        for (matchResult: MatchResult in regex.findAll(text)) {
            ...
            if (!matchWithin && start < cursorOffset)
                continue
            if (!matchBefore && start > cursorOffset)
                continue
...
fun regexExecutor(category: String, group: String, pattern: String, replacementPattern: String, matchWithin: Boolean = false, matchBefore: Boolean = true): Executor {
    return Executor(
        category,
        group,
        pattern,
