...
open class Executor(
    ...
    open fun findMatch(text: String, cursorOffset: Int, reverse: Boolean): Match? {
        for (matchResult: MatchResult in regex.findAll(text)) {
            ...
            // Caret can be in the middle of the matched word. Match can start before the caret but cannot end before it.
            // tru<caret>e -> OK
            // true<caret> -> NOT OK
            ...
            var replacement = (if (reverse) backwardTransform(matchResult) else forwardTransform(matchResult)) ?: continue
            ...
            if (preserveCase) {
                ...
                else if (matchResult.value.firstOrNull()?.isUpperCase() == true && matchResult.value.substring(1).all { it.isLowerCase() })
...
fun wordSet(category: String, group: String, vararg words: String, wholeWords: Boolean = true): Executor {
    return buildWordSetExecutors(category, group, words = words, wholeWords = wholeWords)
}

fun normalizedCaseWordSet(category: String, group: String, vararg words: String, wholeWords: Boolean = true): Executor {
    return buildWordSetExecutors(category, group, words = words, wholeWords = wholeWords, preserveCase = true)
}